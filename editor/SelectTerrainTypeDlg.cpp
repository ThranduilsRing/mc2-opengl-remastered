/***************************************************************
* FILENAME: SelectTerrainTypeDlg.cpp
* DESCRIPTION: Implements the Editor terrain-type selection dialog.
* AUTHOR: Microsoft Corporation
* COPYRIGHT: Copyright (C) Microsoft Corporation. All rights reserved.
* DATE: 04/28/2026
* MODIFICATION: by Methuselas
* CHANGES: Terrain type list now populated via EditorResourceCatalog::GetTerrainNameByLegacyStringId/ByIndex with EditorSafeLoadString fallback. Added EditorResourceCatalog.h include.
****************************************************************/
...
BOOL SelectTerrainTypeDlg::OnInitDialog() 
{
	...
	for ( i = 0; i < numTerrains; i++ )
	{
		...
		{
			char buffer[256];
			long nameID = land->terrainTextures->getTextureNameID(i);
			// Use catalog key lookup first; fall back to legacy EditorSafeLoadString.
			if (!EditorResourceCatalog::GetTerrainNameByLegacyStringId(nameID, buffer, 256))
			{
				if (!EditorResourceCatalog::GetTerrainNameByIndex(i, buffer, 256))
