...
## 0.16.0-editor — Final cleanup

- Removed traces from Editor code.
- Set Editor to open full-screen as default.
- Added esplash.bmp and tacsplash.bmp to root.
- Bumped Editor SemVer to `0.16.0-editor` in `Editor/EditorVersion.h`
  and `Editor/EditorVersion.cpp`.

- Repaired the Editor terrain shader runtime path against the v0.3 renderer changes.
- Fixed GLSL shader-source assembly during runtime compilation.
- Removed duplicate `#version` injection during shader build.
- Repaired shader compatibility issues caused by reserved GLSL identifiers.
- Restored successful terrain material creation inside the Editor renderer path.
- Restored the tessellated terrain shader render path in the Editor.
- Confirmed Editor terrain rendering now matches the active MC2R runtime shader path.
- Migrated all Editor strings from mc2res.dll to /data/defs.
- Bumped Editor SemVer to `0.15.0-editor` in `Editor/EditorVersion.h`
  and `Editor/EditorVersion.cpp`for v0.3 shader compatibility.
