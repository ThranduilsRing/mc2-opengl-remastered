...
{
	EditorTrace("Editor::init: enter loader=%s", loader ? loader : "NULL");

	volatile float crap = 0;
	bool bOK = false;

	EditorTrace("Editor::init: before EditorCamera check eye=%p", eye);
	if ( !eye )
	{
		EditorTrace("Editor::init: before new EditorCamera");
		eye = new EditorCamera;
		EditorTrace("Editor::init: after new EditorCamera eye=%p", eye);
	}
	EditorTrace("Editor::init: after EditorCamera check");

	EditorTrace("Editor::init: before Pilot::initPilots");
	Pilot::initPilots();
	EditorTrace("Editor::init: after Pilot::initPilots");

	FullPathFileName bdgFileName;
	EditorTrace("Editor::init: before Buildings csv path init");
	bdgFileName.init(artPath,"Buildings",".csv");
	EditorTrace("Editor::init: after Buildings csv path init path=%s", (const char*)bdgFileName);

	FullPathFileName objFileName;
	EditorTrace("Editor::init: before Object2 pak path init");
	objFileName.init(objectPath,"Object2",".pak");
	EditorTrace("Editor::init: after Object2 pak path init path=%s", (const char*)objFileName);

	EditorTrace("Editor::init: before EditorObjectMgr::instance");
	EditorObjectMgr* pMgr = EditorObjectMgr::instance();
	EditorTrace("Editor::init: after EditorObjectMgr::instance ptr=%p", pMgr);

	EditorTrace("Editor::init: before EditorObjectMgr::init");
	pMgr->init( bdgFileName, objFileName );
	EditorTrace("Editor::init: after EditorObjectMgr::init");

	FullPathFileName mPath;
	EditorTrace("Editor::init: before mission path init missionName=%s", missionName);
	mPath.init(missionPath,missionName,".pak");
	EditorTrace("Editor::init: after mission path init path=%s", (const char*)mPath);

	bool bCanceled = false;
	if (!justResaveAllMaps)
	{
		EditorTrace("Editor::init: not justResaveAllMaps");
		EditorTrace("Editor::init: before fileExists mission");
		bool missionExists = fileExists(mPath);
		EditorTrace("Editor::init: after fileExists mission exists=%d", missionExists ? 1 : 0);

		if (missionExists)
		{
			EditorTrace("Editor::init: before EditorData::initTerrainFromPCV existing mission");
			bOK = EditorData::initTerrainFromPCV(mPath);
			EditorTrace("Editor::init: after EditorData::initTerrainFromPCV existing mission bOK=%d", bOK ? 1 : 0);
		}
		else 
		{
			EditorTrace("Editor::init: before NewSingleMission dialog block");
			NewSingleMission dlg;
			bool resolved = false;
			while (!resolved)
			{
				EditorTrace("Editor::init: before NewSingleMission DoModal");
				int retVal = dlg.DoModal();
				EditorTrace("Editor::init: after NewSingleMission DoModal retVal=%d", retVal);

				if ( retVal == IDCANCEL )
				{
					EditorTrace("Editor::init: NewSingleMission canceled");
					if (EditorInterface::instance())
								...
								{
									EditorTrace("Editor migration: skipping SetBusyMode because EditorInterface instance is not constructed yet");
								}
					resolved = true;
					...
					if (EditorInterface::instance())
								...
								{
									EditorTrace("Editor migration: skipping UnsetBusyMode because EditorInterface instance is not constructed yet");
								}
				}
				else if ( retVal == ID_NEWMISSION )
				{
					EditorTrace("Editor::init: NewSingleMission returned ID_NEWMISSION");
					resolved = true;

					while ( !bOK )
					{
						EditorTrace("Editor::init: before TerrainDlg");
						TerrainDlg dlg;
						dlg.terrain = 0;

						EditorTrace("Editor::init: before TerrainDlg DoModal");
						int terrainRet = dlg.DoModal();
						EditorTrace("Editor::init: after TerrainDlg DoModal ret=%d terrain=%d", terrainRet, dlg.terrain);

						if ( IDOK == terrainRet )
						{
							EditorTrace("Editor::init: before MapSizeDlg");
							MapSizeDlg msdlg;
							msdlg.mapSize = 0;
							EditorTrace("Editor::init: before MapSizeDlg DoModal");
							int mapRet = msdlg.DoModal();
							EditorTrace("Editor::init: after MapSizeDlg DoModal ret=%d mapSize=%d", mapRet, msdlg.mapSize);
							...
							if ( IDOK == mapRet )
							{
								...
								EditorTrace("Editor::init: camera path=%s", path);

								FitIniFile camFile;
								EditorTrace("Editor::init: before camera file open");
								long camResult = camFile.open( path );
								EditorTrace("Editor::init: after camera file open result=%ld", camResult);
								if ( NO_ERR != camResult )
								{
									EditorTrace("Editor::init: camera file missing; skipping STOP for debug");
									// STOP(( "Need Camera File " ));
								}

								EditorTrace("Editor::init: before SetBusyMode for new terrain");
								if (EditorInterface::instance())
								{
									EditorInterface::instance()->SetBusyMode();
								}
								...
								EditorTrace("Editor::init: after SetBusyMode for new terrain");

								EditorTrace("Editor::init: before eye->init");
								eye->init( &camFile );
								EditorTrace("Editor::init: after eye->init");

								EditorTrace("Editor::init: before EditorData::initTerrainFromTGA");
								bOK = EditorData::initTerrainFromTGA( msdlg.mapSize, 0, 0, dlg.terrain );
								EditorTrace("Editor::init: after EditorData::initTerrainFromTGA bOK=%d", bOK ? 1 : 0);

								EditorTrace("Editor::init: before UnsetBusyMode for new terrain");
								if (EditorInterface::instance())
								{
									EditorInterface::instance()->UnsetBusyMode();
								}
								...
								EditorTrace("Editor::init: after UnsetBusyMode for new terrain");
							}
							else
							{
								EditorTrace("Editor::init: MapSizeDlg canceled");
								resolved = false;
								break;
							}
						}
						...
						{
							EditorTrace("Editor::init: TerrainDlg canceled");
							resolved = false;
							break;
						}
					}
				...
				{
					EditorTrace("Editor::init: NewSingleMission returned open existing branch retVal=%d", retVal);
					resolved = true;
					...
					while ( !bOK )
					{
						EditorTrace("Editor::init: before CFileDialog DoModal");
						if (  IDOK == fileDlg.DoModal() )
						{
							EditorTrace("Editor::init: after CFileDialog DoModal IDOK");
							if (EditorInterface::instance())
								{
									EditorInterface::instance()->SetBusyMode();
								}
							...
							EditorTrace("Editor::init: before EditorData::initTerrainFromPCV selected file=%s", pFile ? pFile : "NULL");
							bOK = EditorData::initTerrainFromPCV( pFile );
							EditorTrace("Editor::init: after EditorData::initTerrainFromPCV selected bOK=%d", bOK ? 1 : 0);
							if (EditorInterface::instance())
								{
									EditorInterface::instance()->UnsetBusyMode();
								}
						...
						{	
							EditorTrace("Editor::init: CFileDialog canceled");
							resolved = false;
							break;
						}
					}
	...
	{
		EditorTrace("Editor::init: justResaveAllMaps path");
		bOK = true;
	}

	EditorTrace("Editor::init: after mission/new loop bOK=%d bCanceled=%d", bOK ? 1 : 0, bCanceled ? 1 : 0);

	if (bCanceled)
	{
		EditorTrace("Editor::init: bCanceled terrain safety block");
		if ( !land )
		{
			EditorTrace("Editor::init: before canceled Terrain new");
			land = new Terrain( );
			EditorTrace("Editor::init: after canceled Terrain new ptr=%p", land);
			EditorTrace("Editor::init: before canceled land->init");
			land->init( 0, (PacketFile*)NULL, EDITOR_VISIBLE_VERTICES, crap, 100  );
			EditorTrace("Editor::init: after canceled land->init");
		}
	}

	if (bOK)
	{
		EditorTrace("Editor::init: before EditorInterface::instance()->init");
		if (EditorInterface::instance())
	{
	...
	else
	{
		...
		*/
		EditorTrace("Editor migration: skipping EditorInterface::init because EditorInterface instance is not constructed yet");
	}
		EditorTrace("Editor::init: after EditorInterface::instance()->init");
	}

	EditorTrace("Editor::init: before PlaySound");
	PlaySound("SystemDefault",NULL,SND_ASYNC);
	EditorTrace("Editor::init: complete");
}
...
void Editor::update()
{
	...
	{
		EditorTrace("Editor migration: skipping SetBusyMode(false) because EditorInterface instance is not constructed yet");
	}
		land->update();
		if (EditorInterface::instance())
								{
...
void EditorInterface::OnPaint() 
{
	...
	{
		EditorTrace("EditorInterface::OnPaint count=%lu initialized=%d loading=%d land=%p", s_paintCount, bThisIsInitialized ? 1 : 0, bIsLoading ? 1 : 0, land);
	}
...
bool EditorInterface::SafeRunGameOSLogic()
{
	...
	if (bThisIsInitialized && (NULL != land))
	{
		if (!m_bInRunGameOSLogicCall)
		{
			...
			{
				EditorTrace("EditorInterface::SafeRunGameOSLogic run count=%lu", s_safeRunCount);
			}
