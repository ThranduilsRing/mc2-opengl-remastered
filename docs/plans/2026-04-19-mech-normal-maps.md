...
- [ ] **Step 8.1: Update the memory note.**
  ...
  Append to `C:/Users/Joe/.claude/projects/A--Games-mc2-opengl-src/memory/mech_paint_and_mipmap_system.md` a new section describing the normal-map opt-in: per-chassis `_N.tga` convention, `gos_State_Texture2` binding, tangent attribute location, and the `useNormalMap` uniform.
